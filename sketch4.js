            ...
            for (let gridX = 0; gridX < cols; gridX++) {
                if (gridX + (cols * gridY) < languages.length) {
                    ...
                    for (let i = 0; i < bufferLength; i += inc) {
                        ...
                        let amplitude = map(data[i], -1 * maxAmplitudes[(gridX + (cols * gridY)) % (languages.length)], maxAmplitudes[(gridX + (cols * gridY)) % (languages.length)] * 1, -gridHeight * 1.2, gridHeight * 1.2, true);
                        ...
                        finalY = indiOffsetY - 0.5 / 6 * height - amplitude
